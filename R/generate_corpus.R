...
generate_corpus <- function(text, type, sub_token, label = NULL, clean = TRUE) {
  ...
  tokens <- corpus %>% tidytext::unnest_tokens(output = word, input = Text, token = type) %>% dplyr::mutate(Token_Id = row_number())
  tok_counts <- tokens %>% tidytext::unnest_tokens(output = sub, input = word, token = sub_token) %>% dplyr::group_by(Token_Id) %>% dplyr::summarize(N = n())
